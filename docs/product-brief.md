...
- Contes : des contes et histoires chinoises réécrits à chaque seuil avec les seuls caractères du seuil. Le même conte existe en plusieurs versions ; l'utilisateur relit la même histoire, plus riche, à mesure que son acquis grandit. Les versions sont générées par lots ou rédigées dans le pipeline puis relues, jamais dans l'app.
  - Niveaux : chaque conte traditionnel est prévu à deux ou trois seuils selon la richesse du récit d'origine (décision du propriétaire). Un récit simple, un épisode et une chute, en a deux : le plus bas, puis deux seuils plus haut (255 et 505, 405 et 805). Un récit riche, plusieurs épisodes ou retournements, en a trois : le plus bas, 805 et 1555, où il se dit en entier. Le plus bas est le premier seuil où le récit se dit sans perdre son sujet : 255 sans animal ni objet hors liste, 405 pour une fable animalière (la liste 255 n'a aucun nom d'animal), 505 pour une morale abstraite ou le merveilleux. Le catalogue le dit conte par conte (`data/sources/contes/catalogue.tsv`). Un niveau prévu dont la liste n'est pas encore versionnée attend sa liste : `wenlu check` le signale, sans jamais bloquer.
  - Dans Lire, chaque conte du catalogue a sa ligne, et sous son titre ses niveaux en petits sceaux, comme ceux des trophées : plein, écrit et ouvert par l'acquis ; au trait, écrit mais pas encore ouvert ; en pointillés, pas encore écrit. Rien n'est estimé : ce que l'export n'a pas n'est pas écrit. Les contes ouverts d'abord, puis les écrits fermés, puis ceux à venir.
  - Contes longs : un récit long (deux prévus, 木兰从军 d'après 《木兰诗》 et 美猴王 d'après les sept premiers chapitres du 《西游记》, à 405 ou 505 et plus) se lit chapitre par chapitre, chaque chapitre titré, de la longueur d'une fable du même seuil. Le lecteur montre un chapitre à la fois, un sommaire replié, « Chapitre suivant », et reprend au chapitre où l'on s'était arrêté (noté dans la progression, export et import compris). Le conte n'est lu, et son trophée gagné, qu'une fois tous ses chapitres lus : un chapitre non lu ne compte pas. Une fable se lit d'une traite, comme avant.
